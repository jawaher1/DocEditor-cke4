...
/*------------------------------------------ Canvas & Controls ---------------------------------------*/
        ...
        gm.initDefaultButtons = function(){
          if (gm.options.customElementEnabled) {
            gm.options.customControls.global_col.push({callback: gm.addElement, loc: 'top', iconClass: 'fa fa-plus', title: 'Add Cutom Element'});
        }
...
/*------------------------------------------ Add Elements ---------------------------------------*/
    /*
              Callback called when the add elements button is clicked
             ...
             */
      var containerElem;
      gm.addElement = function (container, btn) {

        gm.log("Choose Widget");
        gm.log(btn);
        containerElem = container;
        $('#myModalWidget').modal('show');
        $('.widgetItem').click(function (e) {

          var fnName = this.dataset.type;
          var data = this.dataset.dataentry;

          elem = null;
          window[fnName](window[data], containerElem, gm, elem);
          gm.initNewContentElem(elem);

          $('#myModalWidget').modal('hide');
...
/*------------------------------------------ Useful functions ---------------------------------------*/
    ...
    */
        ...
        // Custom CSS to load
        cssInclude: "http://maxcdn.bootstrapcdn.com/font-awesome/4.1.0/css/font-awesome.min.css",
